...
class InterpolatedImageDataset(Dataset):
  ...
  def __getitem__(self, index) -> Any:
    ...
    lowres = F.resize(_degrade(hires, scale_factor=self.scale, interpolation=self.interpolation, distort=self.distort), antialias=True, size=hires.size()[1:3], interpolation=self.interpolation)
